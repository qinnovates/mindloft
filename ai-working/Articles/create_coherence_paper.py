#!/usr/bin/env python3
"""
Create Detailed Research Paper: The Coherence Metric for Neural Signal Integrity
Extended version with mathematical proofs and expanded analysis
"""
...
def create_detailed_paper():
    ...
    title_run = title.add_run('The Coherence Metric for Neural Signal Integrity')
    title_run.font.size = Pt(24)
    title_run.font.bold = True
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER

    subtitle = doc.add_paragraph()
    subtitle_run = subtitle.add_run('A Mathematical Framework for Quantifying Trustworthiness in Brain-Computer Interface Communications')
...
6. We outline paths for empirical validation and identify open questions for future work.""")
    ...
    doc.add_heading('2.1 The Coherence Metric Formulation', level=2)
    doc.add_paragraph('We define signal coherence as an exponential function of total variance:')

    formula1 = doc.add_paragraph()
    formula1.alignment = WD_ALIGN_PARAGRAPH.CENTER
    f1_run = formula1.add_run('Cₛ = e^(−(σ²ᵩ + σ²τ + σ²ᵧ))')
    f1_run.font.size = Pt(16)
    f1_run.bold = True
...
The exponential form was chosen deliberately to model biological threshold behaviors. Neural systems exhibit sharp transitions—a signal either exceeds the threshold for downstream propagation or it doesn't. The exponential decay captures this behavior: coherence remains high at low variance but collapses rapidly as variance increases.""")
    ...
    doc.add_heading('2.2 Phase Variance: Formal Definition', level=2)
    doc.add_paragraph('Phase variance quantifies timing jitter relative to ongoing neural oscillations. For a signal with n arrival events at times {t₁, t₂, ..., tₙ}, each mapped to phase angles {φ₁, φ₂, ..., φₙ} relative to a reference oscillation:')

    formula2 = doc.add_paragraph()
    formula2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    f2_run = formula2.add_run('σ²ᵩ = (1/n) Σᵢ (φᵢ − φ̄)²')
    f2_run.font.size = Pt(14)
    f2_run.bold = True

    doc.add_paragraph("""The phase angle φᵢ at arrival time tᵢ relative to reference frequency f_ref is:
...
The choice of reference frequency is critical. Fries' Communication Through Coherence hypothesis (2005, 2015) proposes that neural populations communicate effectively only when their gamma rhythms are phase-aligned. A signal arriving at random phase relationships to ongoing gamma oscillations will be gated out by inhibitory interneurons.""")

    # Table: Oscillation bands
    doc.add_paragraph().add_run('Table 1: Neural Oscillation Bands and Phase Tolerance Windows').bold = True

    table1 = doc.add_table(rows=6, cols=5)
    table1.style = 'Table Grid'

    headers1 = ['Band', 'Frequency Range', 'Period', 'STDP-like Window', 'Cognitive Association']
    data1 = [
    ...
    doc.add_heading('2.3 Transport Variance: Formal Definition', level=2)
    doc.add_paragraph('Transport variance captures the cumulative unreliability of the signal pathway. For a pathway with n sequential transmission stages, each with probability pᵢ of successful transmission:')

    formula3 = doc.add_paragraph()
    formula3.alignment = WD_ALIGN_PARAGRAPH.CENTER
    f3_run = formula3.add_run('σ²τ = −ln(∏ᵢ pᵢ) = −Σᵢ ln(pᵢ)')
    f3_run.font.size = Pt(14)
    f3_run.bold = True

    doc.add_paragraph("""This formulation has a natural information-theoretic interpretation. The term −ln(p) is the self-information (surprisal) of an event with probability p. Summing over stages gives the total "surprise" accumulated across the pathway—equivalent to pathway entropy.
...
A notable asymmetry exists for BCI signals: electrodes inject signals directly at the neural interface, bypassing many biological transport stages. This paradoxically increases apparent reliability (lower σ²τ), potentially making synthetic signals appear more coherent than natural ones—a detection blind spot.""")
    ...
    # Table: Transport factors
    doc.add_paragraph().add_run('Table 2: Biological Transport Factors and Reliability Estimates').bold = True

    table2 = doc.add_table(rows=8, cols=4)
    table2.style = 'Table Grid'

    headers2 = ['Transmission Stage', 'Reliability (p)', 'Contribution to σ²τ', 'Primary Failure Mode']
    data2 = [
    ...
    doc.add_heading('2.4 Gain Variance: Formal Definition', level=2)
    doc.add_paragraph('Gain variance measures amplitude stability relative to expected bounds:')

    formula4 = doc.add_paragraph()
    formula4.alignment = WD_ALIGN_PARAGRAPH.CENTER
    f4_run = formula4.add_run('σ²ᵧ = (1/n) Σᵢ ((Aᵢ − Ā) / Ā)²')
    f4_run.font.size = Pt(14)
    f4_run.bold = True

    doc.add_paragraph("""Where Aᵢ is the amplitude of the i-th signal instance and Ā is the expected (baseline) amplitude.
...
For BCI security, gain variance is the most straightforward component to bound in hardware. Amplitude limits can be enforced through analog circuitry that operates even when digital systems are compromised.""")
    ...
    doc.add_heading('3. Information-Theoretic Interpretation', level=1)

    doc.add_heading('3.1 Coherence as Negative Entropy', level=2)
    doc.add_paragraph('The coherence metric can be rewritten in information-theoretic terms:')

    formula5 = doc.add_paragraph()
    formula5.alignment = WD_ALIGN_PARAGRAPH.CENTER
    f5_run = formula5.add_run('Cₛ = e^(−H_total)')
    f5_run.font.size = Pt(14)
    f5_run.bold = True

    doc.add_paragraph("""Where H_total = σ²ᵩ + σ²τ + σ²ᵧ represents total entropy (uncertainty) across the three dimensions.
...
Shannon's channel capacity theorem provides an upper bound on reliable information transfer:
...
Our variance terms collectively determine the effective signal-to-noise ratio. High coherence (low total variance) indicates operation near channel capacity; low coherence indicates operation far below theoretical limits.""")
    ...
    # Table: Entropy mapping
    doc.add_paragraph().add_run('Table 3: Variance Components and Information-Theoretic Analogs').bold = True

    table3 = doc.add_table(rows=4, cols=4)
    table3.style = 'Table Grid'

    headers3 = ['Variance', 'Entropy Type', 'Noise Source', 'Information Loss Mechanism']
    data3 = [
    ...
    doc.add_heading('3.2 Comparison to Wireless Error Correction', level=2)
    doc.add_paragraph("""Modern wireless communication faces mathematically similar challenges. 5G New Radio uses Low-Density Parity-Check (LDPC) codes; 6G research explores polar codes and machine learning decoders. All fundamentally address maintaining signal coherence across noisy channels.

Key parallels and differences:""")

    # Table: BCI vs wireless
    doc.add_paragraph().add_run('Table 4: BCI vs. Modern Wireless Communication Systems').bold = True

    table4 = doc.add_table(rows=8, cols=4)
    table4.style = 'Table Grid'

    headers4 = ['Parameter', '5G NR', '6G (Target)', 'BCI Requirement']
    data4 = [
    ...
    doc.add_paragraph()
    doc.add_paragraph("""The critical difference: error tolerance. In wireless, a dropped packet triggers retransmission. In neural interfaces, a corrupted stimulation signal could cause seizure, involuntary movement, or permanent tissue damage. The coherence threshold must be set conservatively, accepting higher false-rejection rates to minimize catastrophic failures.""")

    # ========== 4. NEURAL FIREWALL IMPLEMENTATION ==========
    doc.add_heading('4. Neural Firewall Implementation', level=1)

    doc.add_heading('4.1 Hardware Architecture', level=2)
    doc.add_paragraph('For the coherence metric to protect neural tissue, it must be computable in real-time with minimal power overhead. We propose a five-component architecture:')

    # Table: Hardware
    doc.add_paragraph().add_run('Table 5: Neural Firewall Hardware Components').bold = True

    table5 = doc.add_table(rows=7, cols=5)
    table5.style = 'Table Grid'

    headers5 = ['Component', 'Function', 'Implementation', 'Power', 'Latency']
    data5 = [
    ...
    for i, h in enumerate(headers5):
        ...
        set_cell_shading(table5.rows[0].cells[i], '2F5496')
        table5.rows[0].cells[i].paragraphs[0].runs[0].font.color.rgb = RGBColor(255, 255, 255)

    for row_idx, row_data in enumerate(data5, 1):
        for col_idx, val in enumerate(row_data):
            table5.rows[row_idx].cells[col_idx].text = val
        if row_idx == 6:
            for cell in table5.rows[row_idx].cells:
                cell.paragraphs[0].runs[0].bold = True

    doc.add_paragraph()

    doc.add_heading('4.2 Decision Matrix', level=2)
    doc.add_paragraph('The firewall combines coherence score with authentication status to determine action:')

    # Table: Decision matrix
    doc.add_paragraph().add_run('Table 6: Firewall Decision Matrix').bold = True

    table6 = doc.add_table(rows=6, cols=4)
    table6.style = 'Table Grid'

    headers6 = ['Coherence Level', 'Authentication', 'Action', 'Logging']
    data6 = [
    ...
    for i, h in enumerate(headers6):
        ...
        set_cell_shading(table6.rows[0].cells[i], '2F5496')
        table6.rows[0].cells[i].paragraphs[0].runs[0].font.color.rgb = RGBColor(255, 255, 255)

    for row_idx, row_data in enumerate(data6, 1):
        for col_idx, val in enumerate(row_data):
            table6.rows[row_idx].cells[col_idx].text = val

    doc.add_paragraph()

    doc.add_heading('4.3 Power Budget Analysis', level=2)
    doc.add_paragraph("""For a Neuralink-class device with 25 mW total power budget:
...
Our proposed 2.8 mW architecture fits within this constraint with margin for thermal variation and battery aging. The key insight is that most variance computation can be performed on accumulated statistics (means, variances) rather than raw signals, dramatically reducing computational requirements.""")
    ...
    # ========== 5. SECURITY ANALYSIS ==========
    doc.add_heading('5. Security Analysis', level=1)

    doc.add_heading('5.1 Threat Model', level=2)
    doc.add_paragraph("""We consider adversaries with varying capabilities:
...
The coherence metric provides different levels of protection against each tier.""")
    ...
    doc.add_heading('5.2 Security Strengths', level=2)
    doc.add_paragraph("""1. Phase-lock requirement: Random-phase injection attacks are naturally rejected. A Tier 1 adversary without knowledge of ongoing neural oscillations cannot reliably deliver phase-aligned signals.
...
4. Orthogonal to cryptography: Coherence verification complements rather than replaces cryptographic authentication. Both must pass for signal acceptance.""")
    ...
    doc.add_heading('5.3 Security Vulnerabilities', level=2)
    doc.add_paragraph("""1. Phase-synchronized attacks: A Tier 3 adversary with read access to local field potentials can synchronize malicious stimulation to ongoing rhythms, achieving high coherence scores.
...
4. Slow-drift attacks: Gradual changes in signal parameters might not trigger variance thresholds designed for abrupt anomalies.""")
    ...
    # Table: Attack analysis
    doc.add_paragraph().add_run('Table 7: Attack Vector Analysis').bold = True

    table7 = doc.add_table(rows=6, cols=4)
    table7.style = 'Table Grid'

    headers7 = ['Attack Type', 'Adversary Tier', 'Coherence Defense', 'Residual Risk']
    data7 = [
    ...
    for i, h in enumerate(headers7):
        ...
        set_cell_shading(table7.rows[0].cells[i], '2F5496')
        table7.rows[0].cells[i].paragraphs[0].runs[0].font.color.rgb = RGBColor(255, 255, 255)

    for row_idx, row_data in enumerate(data7, 1):
        for col_idx, val in enumerate(row_data):
            table7.rows[row_idx].cells[col_idx].text = val

    doc.add_paragraph()

    doc.add_heading('5.4 Defense Recommendations', level=2)
    doc.add_paragraph("""To address identified vulnerabilities:
...
4. Behavioral correlation: Compare coherence patterns against expected cognitive states derived from higher ONI layers (L11-L14). Unexpected combinations trigger alerts.""")
    ...
    # Table: Limitations
    doc.add_paragraph().add_run('Table 8: Framework Limitations and Research Paths').bold = True

    table8 = doc.add_table(rows=6, cols=3)
    table8.style = 'Table Grid'

    headers8 = ['Limitation', 'Impact', 'Research Path']
    data8 = [
    ...
    for i, h in enumerate(headers8):
        ...
        set_cell_shading(table8.rows[0].cells[i], '2F5496')
        table8.rows[0].cells[i].paragraphs[0].runs[0].font.color.rgb = RGBColor(255, 255, 255)

    for row_idx, row_data in enumerate(data8, 1):
        for col_idx, val in enumerate(row_data):
            table8.rows[row_idx].cells[col_idx].text = val

    doc.add_paragraph()

    doc.add_heading('6.2 Future Research Directions', level=2)
    doc.add_paragraph("""1. Empirical validation: Test coherence-behavior correlations in animal models. Does Cₛ predict neural discrimination of authentic vs. artificial signals?
...
5. Neuromorphic implementation: Explore spiking neural network implementations that might compute coherence more efficiently than von Neumann architectures.""")
    ...
    doc.add_paragraph("""The Coherence Metric provides a mathematical framework for quantifying neural signal trustworthiness:

Cₛ = e^(−(σ²ᵩ + σ²τ + σ²ᵧ))

Three dimensions—timing, structure, amplitude. One score—trustworthiness.

This formulation connects to information theory (coherence as negative entropy), parallels wireless error correction frameworks, and can be implemented within implantable device power constraints. It provides a quantitative foundation for the Neural Firewall concept within the ONI (Organic Neural Firewall) framework.
...
The brain has been solving the signal integrity problem for 500 million years of evolution. We are formalizing what it already knows—and extending it to defend against threats evolution never anticipated.""")
    ...
    doc.add_paragraph("""1. Fries, P. (2005). A mechanism for cognitive dynamics: neuronal communication through neuronal coherence. Trends in Cognitive Sciences, 9(10), 474-480.

2. Fries, P. (2015). Rhythms for cognition: communication through coherence. Neuron, 88(1), 220-235.

3. Markram, H., Lübke, J., Frotscher, M., & Bhana, B. (1997). Regulation of synaptic efficacy by coincidence of postsynaptic APs and EPSPs. Science, 275(5297), 213-215.

4. Turrigiano, G. G. (2008). The self-tuning neuron: synaptic scaling of excitatory synapses. Cell, 135(3), 422-435.

5. Shadlen, M. N., & Newsome, W. T. (1994). Noise, neural codes and cortical organization. Current Opinion in Neurobiology, 4(4), 569-579.

6. Shannon, C. E. (1948). A mathematical theory of communication. Bell System Technical Journal, 27(3), 379-423.

7. Buzsáki, G., & Draguhn, A. (2004). Neuronal oscillations in cortical networks. Science, 304(5679), 1926-1929.

8. Richardson, A. G., et al. (2019). Wireless, closed-loop neural interfaces: a review. Frontiers in Neuroscience, 13, 1028.

9. Musk, E., & Neuralink. (2019). An integrated brain-machine interface platform. Journal of Medical Internet Research.

10. MITRE Corporation. ATT&CK Framework. https://attack.mitre.org/

11. 3GPP. (2020). 5G NR Physical Layer Specifications. TS 38.211.

12. FDA. (2021). Guidance for Brain-Computer Interface Devices. 21 CFR Part 820.""")

    # Save
    output_path = '/Users/mac/Research/Articles/Coherence_Metric_Detailed_Paper.docx'
