...
Current BCI security treats the interface as a classical digital system. But the interface sits at the boundary of quantum and classical physics. Neural tissue operates at scales where quantum effects are non-negligible.

## The Security Gap

Classical anomaly detection operates at millisecond/millivolt resolution. Quantum effects at the BCI-tissue interface operate at femtosecond/nanovolt scales. An attacker exploiting quantum-scale phenomena is invisible to classical detectors.
...
Davydov solitons are quantum quasiparticles propagating along protein alpha-helices that can trigger synaptic vesicle release via tunneling. Precisely tuned terahertz radiation targeting SNARE protein complexes could trigger false synaptic events — entirely invisible to classical anomaly detection.
...
unknowns = [
    ...
    ["Silicon-tissue boundary", "No quantum framework exists", "L8 Neural Gateway", "Layer 8"],
...
# The QIF Layer Architecture (v2.0)

```{python}
#| label: tbl-layers
#| tbl-cap: "QIF 14-Layer Architecture"
...
```{python}
#| label: fig-layers
#| fig-cap: "QIF 14-Layer Stack: OSI (blue), Neural Gateway (red), Neural (green)"
...
for layer_info in LAYERS:
    ...
    if layer_info['layer'] == 8: color = '#e74c3c'
    w = 0.55 if layer_info['layer'] == 8 else 0.8
    rect = plt.Rectangle(((0.8-w)/2, y), w, 0.85, facecolor=color, alpha=0.85, edgecolor='white', linewidth=2)
    ax.add_patch(rect)
    ax.text(0.4, y+0.42, f"L{layer_info['layer']}: {layer_info['name']}", ha='center', va='center', fontsize=9, fontweight='bold', color='white')

ax.text(0.95, 10, 'OSI', ha='left', fontsize=10, color='#3498db', fontweight='bold')
ax.text(0.95, 6.5, 'GATEWAY', ha='left', fontsize=10, color='#e74c3c', fontweight='bold')
ax.text(0.95, 3, 'Neural', ha='left', fontsize=10, color='#2ecc71', fontweight='bold')
ax.set_xlim(-0.1, 1.3); ax.set_ylim(-0.5, 14.5); ax.axis('off')
plt.tight_layout(); plt.show()
```
...
1. **Unforgeable:** depends on quantum tunneling
...
```{python}
#| label: fig-zeno
#| fig-cap: "Without Zeno: exponential decay of quantum coherence. With Zeno (1kHz sampling): coherence is repeatedly stabilized by measurement."
...
pd.DataFrame(THREAT_MODEL).rename(columns={
    'attack': 'Attack Type', 'layers': 'Layer(s)',
